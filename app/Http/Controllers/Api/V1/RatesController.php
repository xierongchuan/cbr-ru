...
class RatesController extends Controller
{
    ...
    public function index(Request $request): JsonResponse
    {
        ...
        if ($charCodes) {
            $charCodes = is_string($charCodes) ? explode(',', $charCodes) : $charCodes;
        }
        ...
        $rates = Rate::where('date', $date)
            ...
            ->when($charCodes, function ($query) use ($charCodes) {
                $query->whereHas('currency', fn ($q) => $q->whereIn('char_code', $charCodes));
            })
            ->get()
            ->map(fn (Rate $rate) => [
        ...
        $result = ['date' => $date->toDateString(), 'rates' => $rates->toArray()];
